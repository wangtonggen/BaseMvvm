 ...
 *   数据实体类也不存继承问题
 *   当有多种条目的时候，避免在convert()中做太多的业务逻辑，把逻辑放在对应的 BaseBinder/BaseViewDataBinder 中。
 *   适用于以下情况：
 *   1、实体类不方便扩展，此Adapter的数据类型可以是任意类型，默认情况下不需要实现 getItemType
 *   2、item 类型较多，在convert()中管理起来复杂
 *
 *   ViewHolder 由 [BaseBinder/BaseViewDataBinder] 实现，并且每个[BaseBinder/BaseViewDataBinder]可以拥有自己类型的ViewHolder类型。
 */
